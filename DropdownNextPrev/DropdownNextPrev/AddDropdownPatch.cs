...
public static class AddDropdownPatch
{
        ...
        SelectionChangedDelegate onSelectionChanged, ElementBounds bounds, CairoFont font, string key)
    {
        var dropdown = new GuiElementDropdownWithNavigation(composer.Api, values, names, selectedIndex,
            onSelectionChanged,
            bounds, font);
        var leftArrowBounds = ElementBounds.Fixed(0 - 22, 0, 20, bounds.fixedHeight);
        var rightArrowBounds = ElementBounds.Fixed(bounds.fixedWidth + 2, 0, 20, bounds.fixedHeight);
        ...
        var buttonStyle = EnumButtonStyle.None;
